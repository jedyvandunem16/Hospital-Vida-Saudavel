...
function renderHospitais(hospitais) {
  ...
  if (info) info.textContent = `${hospitais.length} hospital${hospitais.length > 1 ? 's' : ''} encontrado${hospitais.length > 1 ? 's' : ''}`;
  ...
  grid.innerHTML = hospitais.map(h => {
    ...
    const isAvail = h.id !== 2;

    return `
      <div class="hospital-card" data-id="${h.id}" onclick="abrirModal(${h.id})">
        <div class="hospital-card-img">
          <img src="${h.foto_url || 'img/hospital-geral-luanda.jpg'}"
               alt="${h.nome}" loading="lazy"
               onerror="this.src='img/hospital-geral-luanda.jpg'"/>
        ...
        <div class="hospital-card-body">
          <div class="hospital-header-info">
            <h3>${h.nome}</h3>
            ${isAvail ? `<span class="avail-badge"><i class="fa-solid fa-bolt"></i> Disp. hoje</span>` : `<span class="unavail-badge"><i class="fa-solid fa-clock"></i> Fila longa</span>`}
          </div>
...
async function abrirModal(id) {
  ...
  content.innerHTML = `<div style="text-align:center;padding:40px;color:#718096"><i class="fa-solid fa-circle-notch fa-spin" style="font-size:1.5rem;color:#1565c0"></i></div>`;
  ...
  try {
    ...
    content.innerHTML = `
      <div class="modal-hospital-img">
        <img src="${h.foto_url || 'img/hospital-geral-luanda.jpg'}" alt="${h.nome}"
              onerror="this.src='img/hospital-geral-luanda.jpg'"/>
      </div>
